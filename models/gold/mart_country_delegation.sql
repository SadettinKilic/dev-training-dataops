...
SELECT
    ...
    ROUND(COUNT(DISTINCT a.code) / NULLIF(COUNT(DISTINCT c.code), 0), 2)
        AS athlete_to_coach_ratio
FROM {{ ref('stg_nocs') }} AS n
LEFT JOIN {{ ref('stg_athletes') }} AS a ON n.code = a.country_code
LEFT JOIN {{ ref('stg_coaches') }} AS c ON n.code = c.country_code
GROUP BY 1
